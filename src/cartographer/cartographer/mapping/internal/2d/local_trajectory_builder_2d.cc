...
std::unique_ptr<transform::Rigid2d> LocalTrajectoryBuilder2D::ScanMatch(
  ...
  // the Ceres scan matcher.   ceres优化参数的初值
...
// invoke by GlobalTrajectoryBuilder::AddRangerData
...
LocalTrajectoryBuilder2D::AddRangeData(
  ...
  for (size_t i = 0; i < synchronized_data.ranges.size(); ++i) {
    ...
    //           synchronized_data.origins.at(synchronized_data.ranges[i].origin_index);
    // hhh 将点云转到以小车起点为原点的global frame,由于对每个点使用不用的tf,即
    // hit_in_local为运动补偿点云 即 相当于对点云的每个点做了畸变校正
  ...
  if (num_accumulated_ >= options_.num_accumulated_range_data()) {
    ...
    return AddAccumulatedRangeData(
        time,
        // hhh 将点云反转回去(但origin为最后一个点的global frame)
        //并对点云使用gravity_alignment进行铅锤校正
...
LocalTrajectoryBuilder2D::AddAccumulatedRangeData(
  ...
  // hhh gravity aligned pose prediction,保证二维平面内的位姿变换
  ...
  // Matching!!!  local map frame <- gravity-aligned frame
  ...
  const transform::Rigid3d pose_estimate =  //取消gravity aligned,复原pose
      transform::Embed3D(*pose_estimate_2d) * gravity_alignment;
  extrapolator_->AddPose(time, pose_estimate);

  sensor::RangeData range_data_in_local =  // global frame
...
LocalTrajectoryBuilder2D::InsertIntoSubmap(
  ...
  //运动过小时 motion_filter_.IsSimilar 将返回true
...
// invoke by GlobalTrajectoryBuilder::AddSensorData
void LocalTrajectoryBuilder2D::AddImuData(const sensor::ImuData& imu_data) {
  CHECK(options_.use_imu_data()) << "An unexpected IMU packet was added.";
  InitializeExtrapolator(imu_data.time);  // extrapolator_不为nullptr时直接返回
