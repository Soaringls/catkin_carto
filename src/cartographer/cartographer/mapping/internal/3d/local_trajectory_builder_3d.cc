...
LocalTrajectoryBuilder3D::AddRangeData(
  ...
  // hhh 将点云转到global frame,即以小车起始原点的全局坐标系下
  ...
  if (num_accumulated_ >= options_.num_accumulated_range_data()) {
    ...
    return AddAccumulatedRangeData(
        current_sensor_time,
        // hhh 将点云反转回去(但原点为global frame 即current_pose的t部分)
...
LocalTrajectoryBuilder3D::AddAccumulatedRangeData(
  ...
  const transform::Rigid3d
      pose_prediction =  //以小车起始原点的全局坐标系下，即local坐标系
...
LocalTrajectoryBuilder3D::InsertIntoSubmap(
  ...
  std::vector<std::shared_ptr<const mapping::Submap3D>>
      insertion_submaps =  // insertion_submaps最大为2
      active_submaps_.InsertRangeData(
          filtered_range_data_in_local,  //以起点为原点的全局local坐标系
          gravity_alignment);
